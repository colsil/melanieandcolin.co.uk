...
if [ "${TRAVIS_PULL_REQUEST}" = "false" ] &&  [ "${TRAVIS_PHP_VERSION}" = "7.0" ]  && [ "${TRAVIS_BRANCH}" = "master" ]; then
    ...
    ssh -i build/sshkeys/travis_key.rsa ubuntu@melanieandcolin.uk "rm -rf /tmp/melanieandcolin.co.uk && mkdir /tmp/melanieandcolin.co.uk && tar -xzf site.tar.gz -C /tmp/melanieandcolin.co.uk"
    ssh -i build/sshkeys/travis_key.rsa ubuntu@melanieandcolin.uk "rsync -azv --exclude "parameters.yml" --exclude "cache" --exclude "sessions" --delete /tmp/melanieandcolin.co.uk/ /var/www/melanieandcolin.co.uk/
